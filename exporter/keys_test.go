...
func TestCheckKeysMultipleDBs(t *testing.T) {
	...
	e, _ := NewRedisExporter(uri,
		Options{Namespace: "test",
			CheckSingleKeys: "single," +
				dbNumStr + "=" + keys[0] + "," +
				dbNumStr + "=" + singleStringKey + "," +
				altDBNumStr + "=" + TestKeysHllName + "," +
				altDBNumStr + "=" + singleStringKey + "," +
				anotherAltDbNumStr + "=" + keys[0],
	...
	for _, k := range []string{
		...
		fmt.Sprintf(`test_key_size{db="db%s",key="%s"} 16`, dbNumStr, singleStringKey),
		...
		fmt.Sprintf(`test_key_value{db="db%s",key="%s"} 1234.56`, dbNumStr, keys[0]),
		fmt.Sprintf(`test_key_value{db="db%s",key="%s"} 1234.56`, anotherAltDbNumStr, keys[0]),

		fmt.Sprintf(`key_memory_usage_bytes{db="db%s",key="%s"}`, dbNumStr, singleStringKey),
		fmt.Sprintf(`key_memory_usage_bytes{db="db%s",key="%s"}`, altDBNumStr, singleStringKey),
		fmt.Sprintf(`key_memory_usage_bytes{db="db%s",key="%s"}`, anotherAltDbNumStr, keys[0]),
	} {
