...
func getStringInfoNotPipelined(c redis.Conn, key string) (keyInfo, error) {
	...
	if info.strVal, err = redis.String(doRedisCmd(c, "GET", key)); err != nil {
...
func (e *Exporter) getKeyInfo(ch chan<- prometheus.Metric, c redis.Conn, dbLabel string, keyType string, keyName string) {
	...
	case "string":
		info, err = getStringInfoNotPipelined(c, keyName)
	case "list":
		if size, err = redis.Int64(doRedisCmd(c, "LLEN", keyName)); err == nil {
			info.size = float64(size)
		}
	case "set":
		if size, err = redis.Int64(doRedisCmd(c, "SCARD", keyName)); err == nil {
			info.size = float64(size)
		}
	case "zset":
		if size, err = redis.Int64(doRedisCmd(c, "ZCARD", keyName)); err == nil {
			info.size = float64(size)
		}
	case "hash":
		if size, err = redis.Int64(doRedisCmd(c, "HLEN", keyName)); err == nil {
			info.size = float64(size)
		}
	case "stream":
		if size, err = redis.Int64(doRedisCmd(c, "XLEN", keyName)); err == nil {
			info.size = float64(size)
		}
	default:
		err = fmt.Errorf("unknown type: %v for key: %v", info.keyType, keyName)
	}
	...
	e.registerConstMetricGauge(ch, "key_size", info.size, dbLabel, keyName)

	// Only run on single value strings
	if info.keyType == "string" && !e.options.DisableExportingKeyValues && info.strVal != "" {
		if val, err := strconv.ParseFloat(info.strVal, 64); err == nil {
...
func getKeysFromPatterns(c redis.Conn, keys []dbKeyPair, count int64) (expandedKeys []dbKeyPair, err error) {
	...
	for _, k := range keys {
		if globPattern.MatchString(k.key) {
			...
			log.Errorf("keyNames: %#v", keyNames)
