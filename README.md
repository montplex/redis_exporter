...
| include-engula-metrics  | REDIS_EXPORTER_INCL_ENGULA_METRICS     | Whether to scrape Engula metrics from `ENGULA INFO`, defaults to true. Has no effect on a server without the command. |
| include-engula-debug-metrics | REDIS_EXPORTER_INCL_ENGULA_DEBUG_METRICS | Whether to also collect Engula's developer tier, by scraping `ENGULA INFO everything` instead of `ENGULA INFO`. Roughly 3x the series; defaults to false. |
